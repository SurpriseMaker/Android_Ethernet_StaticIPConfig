...
public class EthernetNetworkFactory extends NetworkFactory {
    ...
    public EthernetNetworkFactory(Handler handler, Context context, NetworkCapabilities filter) {
        ...
        if (DBG) {
            Log.d(TAG, "EthernetNetworkFactory new");
        }
        mHandler = handler;
        mContext = context;
    ...
    protected void needNetworkFor(NetworkRequest networkRequest, int score) {
        ...
        if (network == null) {
            Log.e(TAG, "needNetworkFor, failed to get a network for start" + networkRequest);
            return;
        }

        if (++network.refCount == 1) {
            Log.d(TAG, "+needNetworkFor,  " + networkRequest);
    ...
    protected void releaseNetworkFor(NetworkRequest networkRequest) {
        NetworkInterfaceState network = networkForRequest(networkRequest);
        if (network == null) {
            Log.e(TAG, "needNetworkFor, failed to get a network for stop" + networkRequest);
            return;
        }
    ...
    void removeInterface(String interfaceName) {
        ...
        if (DBG) {
            Log.d(TAG, "removeInterface iface=" + iface + " interfaceName=" + interfaceName);
        }
        if (iface != null) {
            iface.stop();
        }
