...
final class EthernetTracker {
    ...
    void start() {
        if (DBG) {
            Log.d(TAG, "EthernetTracker start");
        }
        mConfigStore.read();
    ...
    void enable(){
        if (DBG) {
            Log.d(TAG, "EthernetTracker enable");
        }
        ...
        mEnabled = true;
        mHandler.post(this::trackAvailableInterfaces);
        if(isTrackingInterface(Settings.Global.INTERFACE_ETH0)){
            updateInterfaceState(Settings.Global.INTERFACE_ETH0,true);
        }
        
        if(isTrackingInterface(Settings.Global.INTERFACE_ETH1)){
            updateInterfaceState(Settings.Global.INTERFACE_ETH1,true);
        }
    }
    
    void disable(){
        if (DBG) {
            Log.d(TAG, "EthernetTracker disable");
        }
        updateInterfaceState(Settings.Global.INTERFACE_ETH0,false);
        updateInterfaceState(Settings.Global.INTERFACE_ETH1,false);
    ...
    private void addInterface(String iface) {
        ...
        if(iface.equals(Settings.Global.INTERFACE_ETH0)){
            return;
        }

        if(iface.equals(Settings.Global.INTERFACE_ETH1)){
            return;
        }
