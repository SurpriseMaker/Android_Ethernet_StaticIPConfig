...
//        }
            ...
            if (satisfiesMobileMultiNetworkCheck) {
                ...
                if (currentNetwork == null ||
                    isBestMobileMultiNetwork(currentNetwork,
                          ...
                          newNetwork.networkCapabilities,
                          nri.request.networkCapabilities) ||
                    isWifiEthernetMultiNetwork(currentNetwork,newNetwork) ||
                    currentNetwork.getCurrentScore() < score) {
                    if (VDBG) log("rematch for " + newNetwork.name());
                    if (currentNetwork != null) {
                        if (VDBG) log("   accepting network in place of " + currentNetwork.name());
                        currentNetwork.removeRequest(nri.request.requestId);
                        if(isWifiEthernetMultiNetwork(currentNetwork,newNetwork)){
                            currentNetwork.lingerRequest(nri.request, now, 2 * 60 * 60 * 1000);
                        }else{
    ...
    private boolean isEthernetNetwork(NetworkAgentInfo nai) {
        if (nai != null && nai.networkCapabilities != null &&
    ...
    private boolean isWifiNetwork(NetworkAgentInfo nai){
        if (nai != null && nai.networkCapabilities != null &&
            nai.networkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_ETHERNET)) {
            return true;
        }
        return false;
    }
    
    private boolean isWifiEthernetMultiNetwork(NetworkAgentInfo currentNetwork,NetworkAgentInfo newNetwork){
        if(isEthernetNetwork(currentNetwork) && isWifiNetwork(newNetwork)
            || isEthernetNetwork(newNetwork) && isWifiNetwork(currentNetwork)){
            log("isWifiEthernetMultiNetwork currentNetwork= " + currentNetwork + " newNetwork=" + newNetwork);
            return true;
        }
        return false;
    }
